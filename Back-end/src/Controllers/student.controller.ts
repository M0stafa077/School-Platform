...
export const updateStudentInfoBody = async (req: Request, res: Response) => {
    let studentInfo = {} as student | any;
    studentInfo.id = req.body.id;
    ...
    StudentModel.create(studentInfo)
        ...
        .catch((error) => {
            if (error === false) {
                ...
                res.status(500).send(
                    "Error adding the student info, try again later"
                );
            }
        });
};
export const deleteStudent = async (req: Request, res: Response) => {
    try {
        ...
        if (result === true) {
            ...
            res.status(500).send(
                "Error deleting the student info, try again later!"
            ); // error occured
        }
    } catch (error) {
        res.status(500);
